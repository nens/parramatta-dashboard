...
class PlotlyChartComponent extends Component {
  ...
  updateTimeseries() {
    ...
    this.props.tile.data
      ...
      .map(d => {
        ...
        this.props.getTimeseriesEvents(
          d.xy.uuid,
          this.state.start,
          this.state.end,
          {
  ...
  render() {
    ...
    const retrievedTileData = tile.data.map(dataItem => {
      ...
      if (dataItem.xy.type !== "timeseries") return dataItem;
      console.log("retrievedTileData between breakouts 2");
      if (
        !(
          this.props.timeseries[dataItem.xy.uuid] &&
          this.props.timeseriesEvents[dataItem.xy.uuid] &&
          this.props.timeseriesEvents[dataItem.xy.uuid].events
        )
      ) {
        return dataItem;
      }
      ...
      const timeseriesY = this.props.timeseriesEvents[dataItem.xy.uuid].events
      ...
      dataItem.x = timeseriesX;
      dataItem.y = timeseriesY;
      return dataItem;
    });

    console.log("retrievedTileData ", retrievedTileData);
    ...
    return this.props.isFull
      ? this.renderFull(axes, retrievedTileData, combinedEvents, tile)
      : this.renderTile(axes, retrievedTileData, combinedEvents, tile);
  }

  renderFull(axes, retrievedTileData, combinedEvents, tile) {
    ...
    if (this.props.tile.shortTitle === "BLAAAAA PLOTLY TILE") {
      console.log("combinedevents + layout", combinedEvents, layout);
    }

    let tempPlotlyData = this.props.tile.data;
    // if (combinedEvents[0]){
    //   console.log('combinedEvents[0].x;', combinedEvents[0].x)
    //   tempPlotlyData[0].x = combinedEvents[0].x;
    //   tempPlotlyData[0].y = combinedEvents[0].y;
    //   tempPlotlyData[1].x = combinedEvents[0].x;
    //   tempPlotlyData[1].y = combinedEvents[0].y;
    // }
    console.log("tempPlotlyData", tempPlotlyData);
